/*
  ...
  1. New Tables
    - `herd_register` - Main livestock registry with date_of_birth
    - `calf_register` - Calf records linked to parent animals
    - `drug_register` - Drug inventory management
    - `mortality_register` - Cull and mortality records
    - `sales_register` - Sales and purchase transactions
    - `health_records` - Animal health and treatment records
    - `pregnancy_register` - Pregnancy and calving matrix records
...
-- Herd Register Table (updated with date_of_birth)
...
-- Calf Register Table (linked to parent from herd_register)
CREATE TABLE IF NOT EXISTS calf_register (
  ...
  sex text NOT NULL CHECK (sex IN ('male', 'female')),
  birth_weight decimal(6,2) DEFAULT 0,
  weaning_weight decimal(6,2) DEFAULT 0,
  weaning_date date,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  UNIQUE(user_id, tag_number)
...
-- Mortality Register Table
CREATE TABLE IF NOT EXISTS mortality_register (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE,
  animal_tag text NOT NULL,
  date_of_event date NOT NULL,
  event_type text NOT NULL CHECK (event_type IN ('death', 'cull', 'emergency_slaughter')),
  cause text NOT NULL,
  weight decimal(6,2) DEFAULT 0,
  value decimal(10,2) DEFAULT 0,
  disposal_method text NOT NULL CHECK (disposal_method IN ('burial', 'burning', 'rendering', 'veterinary_disposal')),
  notes text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS sales_register (
  ...
  buyer_seller text NOT NULL,
  weight decimal(6,2) DEFAULT 0,
  price_per_kg decimal(10,2) DEFAULT 0,
  total_price decimal(10,2) DEFAULT 0,
  payment_method text NOT NULL CHECK (payment_method IN ('cash', 'bank_transfer', 'check', 'credit')),
  notes text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS breeding_soundness (
  ...
  sperm_morphology decimal(5,2) NOT NULL DEFAULT 0,
  scrotal decimal(5,2) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS heat_detection (
  ...
  body_condition_score decimal(2,1) DEFAULT 0,
...
ALTER TABLE mortality_register ENABLE ROW LEVEL SECURITY;
...
-- RLS Policies for Mortality Register
CREATE POLICY "Users can manage their own mortality records"
  ON mortality_register
  FOR ALL
  TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- RLS Policies for Sales Register
...
CREATE INDEX IF NOT EXISTS idx_mortality_register_user_id ON mortality_register(user_id);
CREATE INDEX IF NOT EXISTS idx_sales_register_user_id ON sales_register(user_id);
